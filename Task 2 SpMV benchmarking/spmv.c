...
my_coo(mat, vec, mysol, size);

timestamp(&now);
printf("Time taken by my my_coo matrix - vector product: %ld ms\n", diff_milli(&start, &now));

if (check_result(refsol, mysol, size) == 1)
    printf("my_coo result is ok!\n");
else
    printf("my_coo result is wrong!\n");

 
 my_csc(mat, vec, mysol, size);

timestamp(&now);
printf("Time taken by my my_csc matrix - vector product: %ld ms\n", diff_milli(&start, &now));

if (check_result(refsol, mysol, size) == 1)
    printf("my_csc result is ok!\n");
else
    printf("my_csc result is wrong!\n");
